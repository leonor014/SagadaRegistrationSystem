...
const subscribeToDocumentCounts = () => {
  const collections = [
    ...
    {
      name: "registrations",
      el: document.querySelector('[data-type="registrations"]'),
    },
    {
      name: "analytics",
      el: document.querySelector('[data-type="analytics"]'),
    },
...
function logDataStructure(docs, sampleSize = 2) {
  ...
  for (let i = 0; i < Math.min(sampleSize, docs.length); i++) {
    ...
    if (
      docs[i].registrationType === "group" &&
      Array.isArray(docs[i].groupMembers)
    ) {
...
function expandRegistrationDocs(docs) {
  ...
  docs.forEach((doc) => {
    ...
    const groupCountry = data.groupMembers?.[0]?.memberCountry || "Unknown";
    const groupRegion = data.groupMembers?.[0]?.memberRegion || "Unknown";
    ...
    if (data.registrationType === "group" && Array.isArray(data.groupMembers)) {
      data.groupMembers.forEach((m) => {
        expanded.push({
          site: "All Sites",
          nationality:
            m.nationality || m.memberNationality || groupCountry || "Unknown",
  ...
  docs.forEach((doc, index) => {
    ...
    if (data.registrationType === "group" && Array.isArray(data.groupMembers)) {
      ...
      expanded.push({
        ...
        dateOfBirth: data.dateOfBirth || null,
        age:
          data.age !== undefined && data.age !== null ? Number(data.age) : null,
      });
    }
  });
...
function drawPieChart(canvasId, title, labels, data, customColors = []) {
  ...
  if (customColors.length > 0) {
    ...
    colors = labels.map((_, i) => {
      const colorSchemes = [
        creativeColors.vibrant,
        creativeColors.earth,
        creativeColors.ocean,
      ];
    ...
    <tbody>
      ${[...siteMap.keys()]
        .map((site) => {
          ...
          const topC = Object.entries(s.countries)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 3)
            .map((c) => c[0] + "(" + c[1] + ")")
            .join(", ");
          const topR = Object.entries(s.regions)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 3)
            .map((r) => r[0] + "(" + r[1] + ")")
            .join(", ");
...
function renderTouristSpotAnalytics(selectedSite, docs) {
  console.log(
    `Rendering analytics for ${selectedSite} with ${docs.length} documents`
  );
...
function recomputeAndRender() {
  ...
  console.log(
    `Recomputing with: type=${currentReportType}, site=${site}, period=${value}, filterType=${type}`
  );
...
onSnapshot(
  query(collection(db, "registrations"), orderBy("dateOfRegistration", "desc")),
  (snap) => {
